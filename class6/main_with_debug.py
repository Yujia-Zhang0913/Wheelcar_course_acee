...
class Playground:
    ...
    def run(self):
        while True:
            ...
            if self.vplanner_midpos_index >= 0:
                ...
                #cal = DWA()
                [self.vx,self.vw],best_traj,all_traj,all_u = vplanner.plan([self.x,self.y,self.theta,self.vx,self.vw],self.dwaconfig,midpos,self.planning_obs)
            else:
    ...
    def __draw(self,best_traj,all_traj,all_value):
        ...
        if best_traj is not None :#and best_traj != []:
            print(best_traj)
    ...
    def on_press(self,event):
        ...
        if(event.key == ' '):
            if self.planning_target is not None and self.planner is not None:
                ...
                self.vplanner_midpos_index = None
                cal = planner(self.planning_obs,[self.x,self.y],self.planning_target,-5,-5,5,5,0.2,Playground.planning_obs_radius+DWAConfig.robot_radius)
                px,py = cal.Process()
