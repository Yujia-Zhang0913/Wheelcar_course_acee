...
class A_star:
    ...
    def __init__(self,obstacles_point, start_point, end_point, planning_minx,planning_miny,planning_maxx,planning_maxy):
        ...
        self.start_point = (int((start_point[0]-planning_minx)/0.2),int((start_point[1]-planning_miny)/0.2))
        self.end_point = (int((end_point[0]-planning_minx)/0.2),int((end_point[1]-planning_miny)/0.2))
        ...
        self.width = int((planning_maxx - planning_minx)/0.2+1)
        self.height = int((planning_maxy - planning_miny)/0.2+1)
        self.map = np.empty([self.height, self.width], dtype=float)
        for i in range(self.height):
            for j in range(self.width):
                self.temp=np.hypot(obstacles_point[:,0]-(i*0.2+planning_minx),obstacles_point[:,1]-(j*0.2+planning_miny))
    ...
    def Process(self):
        ...
        best_path_X=[float(i)*0.2+self.planning_minx for i in best_path_X]
        best_path_Y=[float(i)*0.2+self.planning_miny for i in best_path_Y]
