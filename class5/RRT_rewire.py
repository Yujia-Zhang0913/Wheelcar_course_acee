...
class RRT:
    ...
    class RRT_Tree:
        ...
        def __init__(self,point,father,child,endpoint,startpoint):
            ...
            if father:
                self.value = self.father.value+0.2
            else:
                self.value = 0
    ...
    def search_next(self):
        ...
        while flag:
            ...
            near_node = RRT.RRT_Tree(nearest_node.point+d*0.2,nearest_node,[],nearest_node.endpoint,nearest_node.startpoint)
            for node in node_near_set:
                if near_node.value>node.value+0.2:
                    near_node.father = node
                if near_node.value+0.2<node.value:
    ...
    def if_nearend(self,node):
        ...
        return:
            dist 距离，near_node: 类型：Tree 下一个可探索的顶点
        """
        if np.linalg.norm(node.point-node.endpoint) < 0.2:
            return 1
        return 0
